...
const nginxRuntimeDefaults = readFileSync(
  resolve(dashboardRoot, "docker-entrypoint.d/16-admin-dashboard-defaults.envsh"),
  "utf8",
);
...
describe("admin dashboard delivery contracts", () => {
  ...
  it("builds Vite without browser API credentials before producing the nginx image", () => {
    ...
    expect(dockerfile).toContain("/docker-entrypoint.d/16-admin-dashboard-defaults.envsh");
  ...
  it("configures the admin service with runtime same-origin upstream and token values", () => {
    ...
    expect(composeConfig).toContain('ADMIN_DASHBOARD_TOKEN: "${ADMIN_DASHBOARD_TOKEN:-}"');
    expect(composeConfig).toContain('"3001:80"');
  });

  it("selects the fixed same-origin adapter in production without a browser token", () => {
    expect(mainEntry).toContain("createDashboardAdapterForEnvironment(import.meta.env.PROD)");
