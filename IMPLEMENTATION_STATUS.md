...
- Feature branch UI сохранена в origin как проверяемая история серьёзного этапа; backend telemetry/API разрабатывается отдельно в `codex/feat/admin-telemetry-api` и ещё не развёрнут на HomeNode.
