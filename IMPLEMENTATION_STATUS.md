...
- Status terminal connector work выполняется отдельно в `codex/fix/admin-connector-ports`; implementation commits `ed28e9c` и `6e4278b` прошли TDD, полный admin regression и два task review. Публикация и fast-forward в `main` выполняются только после whole-branch review.
